...
Phase 3B1b2a sanitizes internal previews on the server before readable Posts reach ordinary clients, including editor/composer preview lookup. Circle, Post, Task, Event, Goal, Issue, Proposal, and Funding previews use the same canonical resolver and require every canonical owner Circle to be readable; Post previews reuse central Post/source authorization and Event previews require every canonical host. The resolver returns only renderer-required fields plus a separately derived canonical URL. Missing, malformed, deleted, unsupported, suspended, removed, or inaccessible preview sources lose all preview identity and data, the corresponding identifying content token is replaced with neutral text, and the client has no raw-URL fallback. Shared-original handling remains for Phase 3B1b2b, Circle-mention authorization and neutralization remains for Phase 3B1b2c, and Phase 3B1b3 write hardening remains incomplete.
