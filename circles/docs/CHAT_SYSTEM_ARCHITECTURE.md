...
chatReadStates  
Stores the V2 legacy/non-topic cursor and the immutable migration fallback for topics. The old
`lastReadMessageId` remains as a rolling-deployment compatibility field; V2 uses `legacyLastReadMessageId`.

chatTopicReadStates
Stores persistent per-user, per-topic message-id cursors. Topic replies are never marked read merely because the
conversation route is open.

Important notes:

- DMs and announcement conversations rely on `participants`.
- Groups rely on active records in `chatRoomMembers`.
- Legacy `chatRooms`, `chatMessages`, Matrix ids, and Matrix-shaped message fields may still appear in models or compatibility wrappers, but the current runtime chat source of truth is the five collections above.
...
- `getUnreadCountsForUser` sums unread loose legacy messages and server-derived per-topic counts
- `markConversationRead` upserts only the loose legacy cursor while preserving the topic migration fallback
- `markConversationReadAction` resolves the newest loose legacy message; it does not include topic starters or replies
- `markTopicReadAction` writes the newest topic message actually shown to the user
- `useMongoChat` may advance the loose legacy cursor but cannot advance a topic cursor

Topic and conversation indicators use the same Mongo-authoritative semantics:

- `chatTopicReadStates.lastReadMessageId` is the explicit cursor for a topic starter and its replies
- `chatReadStates.legacyLastReadMessageId` is reserved for loose legacy/non-topic messages
- for an existing user whose topic has no explicit row, the immutable `chatReadStates.topicFallbackMessageId` is the
  migration boundary
- an old null cursor is migrated to the newest message present at or before that row's historical `updatedAt`; a V2 row
  with a null fallback is a genuinely new/read-nothing state
- opening a topic writes the newest message actually returned to that topic view
- opening a conversation advances only the loose legacy cursor
- own messages are excluded from both topic and conversation unread counts
...
- the conversation sidebar badge, top Messages/envelope badge, and topic-local unread badge derive from persistent chat
  cursors; the general notification bell remains a separate notification system
