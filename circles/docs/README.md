# Documentation

Short index for developer-facing Kamooni / Circles docs.

## Deployment

- [Circles deployment](circles-deployment.md)
- [Circles registry deployment](circles-registry-deployment.md)
- [Server setup](server-setup.md)

## Telegram Notifications

- [Telegram notification forwarding](TELEGRAM_NOTIFICATIONS.md)

## Development

- [Dev onboarding](dev-onboarding.md)
- [Creating a new module](create-module.md)
- [Debug playbook](DEBUG_PLAYBOOK.md)
- [Circle affiliations](CIRCLE_AFFILIATIONS.md)
