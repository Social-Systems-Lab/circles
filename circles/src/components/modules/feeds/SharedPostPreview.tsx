...
import { SharedOriginalPreview } from "@/models/models";
import { truncateText } from "@/lib/utils";

type SharedPostPreviewProps = {
    post?: SharedOriginalPreview | null;
    fallbackText?: string;
};
        ...
        <div className="overflow-hidden rounded-xl border border-gray-200 bg-gray-50 transition-colors hover:bg-gray-100/80">
            {post.image?.url ? (
                <div className="relative h-44 w-full bg-gray-100">
                    <Image
                        src={post.image.url}
                        alt={post.image.alt || post.title || "Shared post image"}
                        fill
                ...
                <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                        <div className="text-xs text-gray-500">
                            ...
                            {post.circleName ? ` in ${post.circleName}` : ""}
                        </div>
                        {post.title ? (
